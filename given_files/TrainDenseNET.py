...
def transition(inputs, num_filters , compression_factor , dropout_rate ):
    ...
    x = tf.keras.layers.Conv2D( np.floor( compression_factor * num_feature_maps ).astype( int ) ,
                               kernel_size=(1, 1), use_bias=False, padding='same' , kernel_initializer='he_normal' , kernel_regularizer=tf.keras.regularizers.l2( 1e-5 ) )(x)
    x = tf.keras.layers.Dropout(rate=dropout_rate)(x)
...
x = tf.keras.layers.Conv2D( num_filters , kernel_size=( 3 , 3 ) , use_bias=False, kernel_initializer='he_normal' , kernel_regularizer=tf.keras.regularizers.l2( 1e-5 ) )( inputs )
...
model.compile( loss=tf.keras.losses.categorical_crossentropy ,optimizer=tf.keras.optimizers.Adam( lr=0.00001 ) ,metrics=[ 'acc' ])
model.summary()
